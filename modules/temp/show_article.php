...
if($db->RowCount>0){
	foreach($rows as $row) {
		...
		<div class="wrap-detail">
			...
			<?php
			if($row['name']!='') echo '<h4 class="title-vs" style="font-size:25px;">' . stripslashes($row['name']) . '</h4>';
			...
			if($row['comment']!='') echo '<h4 class="txt-medium f-space15" font-family:Roboto;>' . stripslashes($row['comment']) . '</h4>';
